...
class PrepLig(object):
    def __init__(self, input_mol, mol_format):
        if mol_format == 'smi':
            self.rdkit_mol = Chem.MolFromSmiles(input_mol)
        elif mol_format == 'sdf': 
            # 读取SDF时保留氢原子
            supplier = Chem.SDMolSupplier(input_mol, removeHs=False)
            self.rdkit_mol = next(iter(supplier))
        else:
            raise ValueError(f'mol_format {mol_format} not supported')
        
    def addH(self, polaronly=False, correctforph=True, PH=7): 
        self.rdkit_mol = Chem.AddHs(self.rdkit_mol, addCoords=True)

    def gen_conf(self):
        AllChem.EmbedMolecule(self.rdkit_mol, Chem.rdDistGeom.ETKDGv3())

    @supress_stdout
    def get_pdbqt(self, lig_pdbqt=None):
        preparator = MoleculePreparation()
        preparator.prepare(self.rdkit_mol)
